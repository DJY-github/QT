...
void GaugeRange::drawRangeText(QPainter *painter)
{
    ...
    //绘制左边值
    double sina = qSin(startRad - leftStep * deltaRad);
    double cosa = qCos(startRad - leftStep * deltaRad);
    ...
    //绘制右边值
    sina = qSin(startRad - rightStep * deltaRad);
    cosa = qCos(startRad - rightStep * deltaRad);
...
void GaugeRange::drawValue(QPainter *painter)
{
    int radius = 100;
    painter->save();
    ...
    QFont font;
    font.setPixelSize((radius - textWidth) * 0.8);
    painter->setFont(font);
...
{
    setLeftRightRange(leftValue, rightValue);
}
...
void GaugeRange::setLeftRightRange(int leftValue, int rightValue)
{
    //左边值不能大于右边值
...
void GaugeRange::setValue(int value)
{
...
void GaugeRange::setRangeStyle(const GaugeRange::RangeStyle &rangeStyle)
{
