    ...
    : QWidget(parent)
{
    ...
    m_renderer->loadSourceFile(":/deform/pathdeform.cpp");
    m_renderer->loadDescription(":/deform/pathdeform.html");
...
QPainterPath PathDeformRenderer::lensDeform(const QPainterPath &source, const QPointF &offset)
{
    ...
    double flip = m_intensity / double(100);
    ...
    for (int i=0; i<path.elementCount(); ++i) {
        ...
        double dx = x - m_pos.x();
        double dy = y - m_pos.y();
        double len = m_radius - qSqrt(dx * dx + dy * dy);
