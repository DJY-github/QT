    ...
    : QWidget(parent)
{
    ...
    QWidget *mainGroup = new QWidget(parent);
...
#ifdef Q_WS_QWS
    m_image = QPixmap(":/composition/flower.jpg");
    m_image.setAlphaChannel(QPixmap(":/composition/flower_alpha.jpg"));
#else
    m_image = QImage(":/composition/flower.jpg");
    m_image.setAlphaChannel(QImage(":/composition/flower_alpha.jpg"));
#endif
    m_circle_alpha = 127;
    m_circle_hue = 255;
...
void CompositionRenderer::updateCirclePos()
{
    ...
    double t = (dt.toTime_t() * 1000 + dt.time().msec()) / 1000.0;

    double x = width() / double(2) + (qCos(t*8/11) + qSin(-t)) * width() / double(4);
    double y = height() / double(2) + (qSin(t*6/7) + qCos(t * double(1.5))) * height() / double(4);
...
void CompositionRenderer::paint(QPainter *painter)
{
    ...
    if (usesOpenGL()) {
        ...
        double x_fraction = width()/float(m_pbuffer->width());
        double y_fraction = height()/float(m_pbuffer->height());
