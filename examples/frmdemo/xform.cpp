    ...
    : ArthurFrame(parent)
{
    ...
    m_pixmap = QPixmap(":/affine/bg1.jpg");
...
{
    setRotation(double(r) / 10);
}

void XFormView::changeScale(int s)
{
    setScale(double(s) / 1000);
}

void XFormView::changeShear(int s)
{
    setShear(double(s) / 1000);
}
...
{
    m_scale += e->delta() / double(600);
    m_scale = qMax(double(0.1), qMin(double(4), m_scale));
...
{
    QPointF center(m_pixmap.width() / double(2), m_pixmap.height() / double(2));
...
#endif
    view->loadSourceFile(":/affine/xform.cpp");
    view->loadDescription(":/affine/xform.html");
