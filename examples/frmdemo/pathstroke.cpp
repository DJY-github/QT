...
void PathStrokeControls::createCommonControls(QWidget* parent)
{
    ...
    // Layouts
    QHBoxLayout *capGroupLayout = new QHBoxLayout(m_capGroup);
    capGroupLayout->addWidget(flatCap);
    capGroupLayout->addWidget(squareCap);
    capGroupLayout->addWidget(roundCap);

    QHBoxLayout *joinGroupLayout = new QHBoxLayout(m_joinGroup);
    ...
    QHBoxLayout *pathModeGroupLayout = new QHBoxLayout(m_pathModeGroup);
...
PathStrokeWidget::PathStrokeWidget(QWidget *parent, bool smallScreen)
{
    ...
    m_renderer->loadSourceFile(":/pathstroke/pathstroke.cpp");
    m_renderer->loadDescription(":/pathstroke/pathstroke.html");
...
void PathStrokeRenderer::mousePressEvent(QMouseEvent *e)
{
    ...
    double distance = -1;
    for (int i=0; i<m_points.size(); ++i) {
        double d = QLineF(e->pos(), m_points.at(i)).length();
        ...
        foreach (const QTouchEvent::TouchPoint &touchPoint, points) {
            ...
            case Qt::TouchPointPressed:
            {
                ...
                for (int i=0; i<pointsCount; ++i) {
                    ...
                    double d = QLineF(touchPoint.pos(), m_points.at(i)).length();
